...
<head>
    ...
    <title>Check-In — Buku Tamu Digital</title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .scan-box::before, .scan-box::after {
            ...
            width: 16px; height: 16px; border-color: #10b981; border-style: solid;
        }
        .scan-box::before { top: -2px; left: -2px; border-width: 2px 0 0 2px; }
        .scan-box::after  { bottom: -2px; right: -2px; border-width: 0 2px 2px 0; }
        .scan-corner-tr { position: absolute; top: -2px; right: -2px; width: 16px; height: 16px; border-top: 2px solid #10b981; border-right: 2px solid #10b981; }
        .scan-corner-bl { position: absolute; bottom: -2px; left: -2px; width: 16px; height: 16px; border-bottom: 2px solid #10b981; border-left: 2px solid #10b981; }
        ...
        .manual-link:hover { background: #f8fafc; color: #374151; }
        ...
        .btn-switch:hover { border-color: #10b981; color: #059669; background: #f0fdf4; }
...
<header>
    <div class="header-left">
        ...
        <span class="header-sub">Pusat Pendaftaran Wajah AI</span>
    </div>
    ...
    <div class="scanner-card">
        ...
        <div class="status-side">
            <div class="status-main">
                ...
                <div id="idle-state">
                    <div class="status-icon">
                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#10b981" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M8 14s1.5 2 4 2 4-2 4-2"/><line x1="9" y1="9" x2="9.01" y2="9"/><line x1="15" y1="9" x2="15.01" y2="9"/></svg>
                    </div>
                    <div class="status-title" id="status-title">Menunggu Wajah...</div>
                    <div class="status-desc">Silakan berdiri di depan kamera untuk melakukan check-in otomatis.</div>
                </div>

                <!-- Greeting (shows on success) -->
            ...
            <div class="side-actions">
                ...
                <a href="{{ route('tamu.checkout') }}" class="btn-switch">
...
<script>
    ...
    async function processScan() {
        ...
        try {
            const resp = await fetch('{{ route("tamu.checkin-process") }}', {
                method: 'POST',
            ...
            if (result.success) {
                ...
                const color = result.action === 'retry' ? '#dc2626' : result.action === 'already_checkin' ? '#d97706' : '#6b7280';
    ...
    async function doConfirm(tamu_id, confidence, nama) {
        try {
            const resp = await fetch('{{ route("tamu.checkin-confirm") }}', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
